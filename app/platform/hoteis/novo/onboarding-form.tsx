  ...
  return (
    <form ref={formRef} action={formAction} className="space-y-6">
      ...
      <section data-onboarding-step="2" hidden={step !== 2} className="rounded-[28px] bg-white p-6 shadow-sm ring-1 ring-slate-200/70 md:p-8">
        <h2 className="text-xl font-semibold text-slate-950">Módulos</h2><p className="mt-2 text-sm text-slate-600">O baseline operacional é aplicado automaticamente. Módulos opcionais podem ser habilitados pela Platform após a criação; funcionalidades futuras são somente informativas.</p>
        <div className="mt-6 grid gap-3 md:grid-cols-2">{MODULE_CATALOG.map((module) => { const baseline = BASELINE_MODULE_KEYS.includes(module.key as (typeof BASELINE_MODULE_KEYS)[number]); const comingSoon = module.availability === 'coming_soon'; return <article key={module.key} className="rounded-2xl border border-slate-200 p-4"><div className="flex items-center justify-between gap-3"><div><p className="font-semibold text-slate-900">{module.name}</p><p className="mt-1 text-xs text-slate-500">{MODULE_GROUP_LABELS[module.group]}</p></div><span className={baseline ? 'rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-semibold text-emerald-700' : 'rounded-full bg-blue-50 px-2.5 py-1 text-xs font-semibold text-blue-700'}>{baseline ? 'Incluído' : comingSoon ? 'Em breve' : 'Opcional'}</span></div></article>; })}</div>
      </section>
