...
export function GrandMercurePublicHome({ pageData, language, domainContext, preferSubdomainRoot }: {
  ...
  return <main className="hotel-theme-page grand-mercure-dock-layout min-h-screen" style={theme.cssVars} data-hotel-theme={theme.preset} data-hotel-icon-style={theme.iconStyle}>
    ...
    <div className="grand-mercure-scroll-region mx-auto flex max-w-[1280px] flex-col md:px-6 md:py-7">
      <section style={{ order: order('hero') }} className="relative overflow-hidden bg-[#fbf7ef]/25 px-3 pt-3 pb-5 min-[360px]:px-4 md:rounded-[32px] md:border md:border-[#ddcfbb] md:px-10 md:pt-6 md:pb-9">
      ...
      {visible('quick_info') ? <section style={{ order: order('quick_info') }} className="grand-mercure-access-grid grid grid-cols-3 gap-2 px-2 min-[360px]:gap-2.5 min-[360px]:px-3 md:mt-[-1px] md:gap-5 md:px-8 lg:px-14">
      ...
      {visible('banners') ? <section style={{ order: order('banners') }} className="grand-mercure-banner-zone mt-5 px-3 md:mt-8 md:px-8 lg:px-14"><PromotionalBannerCarousel banners={banners} language={language} showEmptyFallback /></section> : null}

      <div style={{ order: order('banners') + 0.5 }}><GrandMercureBrazilianPillars language={language} /></div>

      {visible('contact') ? <section style={{ order: order('contact') }} className="grand-mercure-help-zone grand-mercure-last-content relative mx-3 mt-4 flex min-h-[82px] items-center gap-3 rounded-[20px] border border-[#dfd2c0] bg-[#fffdf9] py-4 pr-16 pl-4 shadow-[0_12px_30px_-24px_rgba(56,45,29,.42)] md:mx-8 md:mt-6 md:min-h-[96px] md:px-7 lg:mx-14">
      ...
      <footer style={{ order: 100 }} className="hidden py-7 text-center text-[10px] uppercase tracking-[.24em] text-[#82786d] md:block">Powered by <span className="normal-case tracking-normal text-[#3d3934]">LibGuest</span></footer>
    </div>
