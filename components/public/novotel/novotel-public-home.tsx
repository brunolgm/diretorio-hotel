...
export function NovotelPublicHome({ pageData, language, domainContext, preferSubdomainRoot }: {
  ...
  const blocks = {
    hero: <section className="relative min-h-[320px] overflow-hidden rounded-b-[34px] bg-[var(--hotel-hero-background)] px-6 pt-5 pb-14 text-white shadow-[0_30px_70px_-38px_rgba(0,43,92,0.72)] md:min-h-[540px] md:rounded-[34px] md:px-12 md:py-10">
    ...
    quick_info: cardGrid ? <section className={`relative z-10 -mt-6 grid gap-2 px-4 md:-mt-10 md:gap-5 md:px-10 ${cardGrid.containerClassName} ${cardGrid.singleCard ? 'mx-auto w-full max-w-xl' : ''}`}>
    ...
    banners: <section className="mt-5 px-4 md:mt-8 md:px-10"><PromotionalBannerCarousel banners={banners} language={language} showEmptyFallback /></section>,
    announcements: null,
    services: null,
    departments: null,
    policies: null,
    contact: <section className="mx-4 mt-4 rounded-[22px] bg-white p-4 shadow-[0_14px_36px_-24px_rgba(0,43,92,0.3)] ring-1 ring-slate-100 md:mx-10 md:mt-6 md:p-5">
  ...
  return <main className="hotel-theme-page min-h-screen pb-[calc(7.5rem+env(safe-area-inset-bottom))] min-[1025px]:pb-12" style={theme.cssVars} data-hotel-theme={theme.preset} data-hotel-icon-style={theme.iconStyle}>
